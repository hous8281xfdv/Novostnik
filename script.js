...
async function register(firstName, lastName, phone, password) {
    try {
        ...
        if (res.ok) {
            ...
            showToast(error.error || 'Ошибка регистрации', true);
            return false;
        }
    } catch (err) {
...
async function login(phone, password) {
    try {
        ...
        if (res.ok) {
            ...
            const error = await res.json();
            showToast(error.error || 'Неверный телефон или пароль', true);
            return false;
        }
    } catch (err) {
        showToast('Ошибка соединения с сервером', true);
...
document.getElementById('openProposeBtn')?.addEventListener('click', () => { if (!currentUser) { showLoginModal(); return; } document.getElementById('proposeModal').style.display = 'block'; });
